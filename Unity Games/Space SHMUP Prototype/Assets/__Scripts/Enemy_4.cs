...
public class Enemy_4 : Enemy
{
	...
	// Enemy_4 starts offscreen & moves to random point. Then it picks another and repeats until the player has shot it down.
	...
	void Start ()
	{
		...
		// There is already an initial position chosen by Main.SpawnEnemy() so add it to points as the initial p0 & p1
		points[0] = pos ;
		points[1] = pos ;
	...
	// This will override OnCollisionEnter that is part of Enemy.cs
	// Because of how MonoBehaviour declares common functions like OnCollisionEnter(), the override keyword is not necessary.
	void OnCollisionEnter ( Collision coll )
	{
		...
		switch ( other.tag )
		{
			case "ProjectileHero" :
				...
				// Because there was a collision, we're guaranteed there is at least a contacts[0], and ContactPoints have a reference
				// 		to thisCollider, which will be the collider for the part of the Enemy_4 that was hit.
				GameObject goHit = coll.contacts[0].thisCollider.gameObject ;
				Part prtHit = FindPart ( goHit ) ;

				if ( prtHit == null )		// if prtHit wasn't found
				{
					// .. then it's usually because, very rarely, thisCollider on contacts[0] will be the ProjectileHero
					// 		instead of the ship part. If so, just look for otherCollider instead
	...
	bool Destroyed ( Part prt )
	{
		...
		// If prt.health is - or less, returns true (yes, it was destroyed)
		return ( prt.health <= 0 ) ;
	}
