...
public enum TurnPhase { idle , pre , waiting , post , gameOver }
...
public class Bartok : MonoBehaviour
{
	...
	// This field is static to enforce that there is only one current player
	...
	void LayoutGame()
	{
		...
		{
			for ( int j = 0 ;  j < 4 ;  j++ )	// There are always four players
			{
				tCB = Draw() ;	// Draw a card

				// Stagger the draw time a bit. Remember order of operations.
				tCB.timeStart = Time.time + drawTimeStagger * ( i * 4 + j ) ;
				// ^ By setting timeStart before calling AddCard, we override CardBartok.MoveTo() setting timeStart
