...
public class Player
{
	...
	public CardBartok AddCard ( CardBartok eCB )
	{
		...
		if ( type == PlayerType.human )
		{
			...
			// Below is the LINQ call that works on the array of CardBartoks. It is similar to doing
			// a foreach(CardBartok cd in cards) and sorting them by rank. It then returns a sorted array
			cards = cards.OrderBy( cd => cd.rank ).ToArray() ;

			// Convert the array CardBartok[] back to a List<CardBartok>
			hand = new List< CardBartok >( cards ) ;

			// Note: LINQ operations can be a bit slow (like it could take a couple of milliseconds),
	...
	public void FanHand()
	{
		...
		for ( int i = 0 ;  i < hand.Count ;  i++ )
		{
			...
			// Multiplying a Quaternion by a Vector3 rotates that Vector3 by the rotation stored in
			// the Quaternion. The result gives us a vector above [0,0,0] that has been rotated by rot degrees
			pos = rotQ * pos ;

			// Add the base position of the player's hand (which will be at the bottom-center of the fan of cards)
			pos += handSlotDef.pos ;

			// This staggers the cards in the z direction, which isn't visible but does keep their colliders from overlapping
			pos.z = -0.5f * i ;

			// The line below makes sure the card moves immediately if it's not the initial deal at the beginning of the game
